...
if [[ ! -f ${BRAIN_EXTRACTION_MASK} ]];
  then
    ...
    if [[ -f ${EXTRACTION_REGISTRATION_MASK} ]]
      then
        logCmd ${ANTS_SCRIPTS_PATH}/antsBrainExtraction.sh \
          -d ${DIMENSION} \
          ...
          then
            ...
            if [[ ${RUN_QUICK} -ne 0 ]];
              then
                ...
                basecall="${ANTS_SCRIPTS_PATH}/antsRegistrationSyNQuick.sh -d ${DIMENSION} -f ${EXTRACTED_SEGMENTATION_BRAIN}"
...
if [[ ! -s ${OUTPUT_PREFIX}ACTStage3Complete.txt ]] && \
  ...
  if [[ ${ACT_STAGE} -eq 0 ]] || [[ ${ACT_STAGE} -eq 3  ]] ; then # BAStages seg
    ...
    logCmd ${ANTS_SCRIPTS_PATH}/antsAtroposN4.sh \
      -d ${DIMENSION} \
  ...
  then
    ...
    if [[ ${RUN_QUICK} -ne 0 ]];
      then
        ...
        basecall="${ANTS_SCRIPTS_PATH}/antsRegistrationSyNQuick.sh -d ${DIMENSION} -f ${REGISTRATION_TEMPLATE}"
